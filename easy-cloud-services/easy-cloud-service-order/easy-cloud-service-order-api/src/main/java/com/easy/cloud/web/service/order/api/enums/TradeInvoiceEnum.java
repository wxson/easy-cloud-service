package com.easy.cloud.web.service.order.api.enums;

import com.easy.cloud.web.component.core.enums.IBaseEnum;
import lombok.AllArgsConstructor;
import lombok.Getter;

import java.util.Optional;

/**
 * 交易发票类型
 *
 * @author GR
 * @date 2021-8-31 22:26
 */
@Getter
@AllArgsConstructor
public enum TradeInvoiceEnum implements IBaseEnum {
    /**
     * 1->不开发票；2->电子发票；3->纸质发票
     */
    NONE(1, "不开发票"),
    ELECTRONIC(2, "电子发票"),
    PAPER(3, "纸质发票"),
    ;
    /**
     * 编码
     */
    private final int code;
    /**
     * 描述
     */
    private final String desc;

    /**
     * 根据code获取对象
     *
     * @param code 编码
     * @return java.util.Optional<com.easy.cloud.web.service.mj.biz.enums.HuCardRuleEnum>
     */
    public static Optional<TradeInvoiceEnum> getInstanceByCode(int code) {
        for (TradeInvoiceEnum orderStatusEnum : TradeInvoiceEnum.values()) {
            if (code == orderStatusEnum.code) {
                return Optional.of(orderStatusEnum);
            }
        }
        return Optional.empty();
    }
}
