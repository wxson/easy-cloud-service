...
public class CertificationActionServiceImpl implements IActionService {
    ...
    public Object handle(ActionDTO actionDTO) {
        ...
        // 实名认证
        Object code = upmsFeignClientService.certification(User.build().setId(userId).setUserName(actionDTO.getUserName()).setIdentity(actionDTO.getIdentity())).getCode();
    ...
    public Object list(ActionDTO actionDTO) {
        ...
        if (StrUtil.isBlank(user.getUserName()) || StrUtil.isBlank(user.getIdentity())) {
            return goodsDOList.stream()
                    .map(goodsDO -> goodsDO.convertTo(ActionVO.class).setGoodsNo(goodsDO.getNo()).setHandle(true)).collect(Collectors.toList());
        }
